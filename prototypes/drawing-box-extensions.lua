...
if settings.startup["pysimple-graphics"].value then
    for kind,buildings in pairs(tiered_buildings) do
        for name,extension in pairs(buildings) do
            local min = 1
            local max = 4
            if name == "solar-tower-panel" then
                min = 0
                max = 32
            end
            for i=min,max do
